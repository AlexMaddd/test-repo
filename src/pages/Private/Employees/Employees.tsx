...
const Employees = () => {
  ...
  const searchItems = debounce((x: string) => {
    ...
    getData({
      ...
      page: 1,
      size: rowsPerPage,
      orderBy: { fname: "ASC", mname: "ASC", lname: "ASC" },
    });
  });
  ...
  const rowActions = (item: EmployeeType) => {
    return (
      <>
  ...
  const btnClick = (action: string, item?: EmployeeType) => {
    if (action == "Delete") {
      Swal.fire({
        icon: "warning",
        title: "Confirm",
              ...
              <TableBody>
                {employees.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell align="center">{item.mname}</TableCell>
                    <TableCell align="center">{item.lname}</TableCell>
                    <TableCell align="center">{item.date_of_birth}</TableCell>
                    <TableCell align="center">
                      {item.employment_status === 1 ? (
                        <CheckBox />
                      ) : (
