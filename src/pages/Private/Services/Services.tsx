...
const Items = () => {
  ...
  const [selectedData, setSelectedData] = useState<ServiceType>();
  ...
  const btnClick = (action: string, item?: ServiceType) => {
    if (action == "Delete") {
      Swal.fire({
        icon: "warning",
        title: "Confirm",
      ...
      if (action === "Items") setOpenItemsDialog(true);
      else setOpenCRUDDialog(true);
    }
  };

  const rowActions = (data: ServiceType) => {
    return (
      <>
              ...
              <TableBody>
                {data.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell align="center">
                      {item.is_parent ? <CheckBox /> : <CheckBoxOutlineBlank />}
                    </TableCell>
                    {rowActions(item)}
                  </TableRow>
                ))}
